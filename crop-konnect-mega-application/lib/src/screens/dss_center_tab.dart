        ...
        children: [
          ...
          LayoutBuilder(
            builder: (context, c) {
              ...
              final stationField =
                  _WeatherStationDropdownField(controller: controller);
              final farmField = _FarmDropdownField(controller: controller);
          ...
          const SizedBox(height: 14),
          _SoilDataSourceSelector(controller: controller),
          const SizedBox(height: 12),
          if (controller.advisorySoilDataSource == SoilDataSource.defaultSensor) ...[
            LayoutBuilder(
              builder: (context, c) {
                final wide = c.maxWidth > 520;
            ...
            _RecordedSampleField(controller: controller),
          const SizedBox(height: 12),
          ...
          if (!ready) ...[
            ...
            Text(
              controller.advisorySoilDataSource ==
                          SoilDataSource.recordedSample &&
                      controller.advisorySoilSample == null
                  ? 'Choose a crop, a sowing date and a recorded sample to continue.'
...
// ─────────────────────────────────── Farm / weather station dropdowns ──

/// A dropdown styled to match [_SelectorButton] so the selection row reads as
/// one set of controls.
class _DropdownField<T> extends StatelessWidget {
  ...
  /// Null disables the control (e.g. only one option exists).
...
class _FarmDropdownField extends StatelessWidget {
  const _FarmDropdownField({required this.controller});
  ...
  Widget build(BuildContext context) {
    ...
    final selectedId = farms.any((f) => f.id == controller.advisoryFarm.id)
        ? controller.advisoryFarm.id
        : farms.first.id;
    ...
    return _DropdownField<String>(
      ...
      onChanged: farms.length < 2
          ...
          : (id) {
              ...
              if (next != null) controller.setAdvisoryFarm(next);
            },
    );
  }
}

class _WeatherStationDropdownField extends StatelessWidget {
  const _WeatherStationDropdownField({required this.controller});

  final StationDashboardController controller;

  @override
  Widget build(BuildContext context) {
    final loggers = controller.advisoryFarm.loggers;
    final selected = controller.advisoryWeatherStation ??
        (loggers.isEmpty ? null : loggers.first);

    if (selected == null) {
      // No logger installed on this farm — show the state rather than an
      // empty dropdown.
      return _DropdownField<String>(
        ...
        items: const [
          DropdownMenuItem<String>(
            value: 'none',
            child: Text('No logger installed'),
          ),
        ],
        onChanged: null,
      );
    ...
    // Same guard as the farm dropdown: never hand DropdownButton a value that
    // is not in its item list.
    final selectedId =
        loggers.any((l) => l.id == selected.id) ? selected.id : loggers.first.id;
    ...
    return _DropdownField<String>(
      ...
      items: [
        for (final logger in loggers)
          DropdownMenuItem<String>(
            value: logger.id,
            child: Text(logger.name,
                maxLines: 1, overflow: TextOverflow.ellipsis),
          ),
      ],
      onChanged: loggers.length < 2
          ? null
          : (id) {
              for (final logger in loggers) {
                if (logger.id == id) {
                  controller.setAdvisoryWeatherStation(logger);
                  return;
                }
              }
            },
...
// ──────────────────────────────────────────────── Soil data source picker ──

/// Chooses where the DSS request's soil block comes from.
///
/// "Use Default Soil Sensor Values" keeps the request exactly as it has always
/// been sent. "Choose Recorded Soil Sample" swaps in the values of a sample
/// captured with the C-type probe — nothing else in the payload changes.
class _SoilDataSourceSelector extends StatelessWidget {
  const _SoilDataSourceSelector({required this.controller});

  final StationDashboardController controller;

  @override
  Widget build(BuildContext context) {
    final source = controller.advisorySoilDataSource;
    final usingSample = source == SoilDataSource.recordedSample;
    ...
    return Column(
      ...
      children: [
        ...
        _SoilSourceOption(
          ...
          title: 'Use Default Soil Sensor Values',
          subtitle: "Live soil readings from the farm's logger",
          selected: !usingSample,
          onTap: controller.isLoadingAdvisory
              ? null
              : () => controller
                  .setAdvisorySoilDataSource(SoilDataSource.defaultSensor),
        ),
        const SizedBox(height: 8),
        _SoilSourceOption(
          icon: Icons.biotech_rounded,
          title: 'Choose Recorded Soil Sample',
          subtitle: 'Run on a sample captured with the C-type probe',
          selected: usingSample,
          onTap: controller.isLoadingAdvisory
              ? null
              : () => controller
                  .setAdvisorySoilDataSource(SoilDataSource.recordedSample),
        ),
      ],
    );
  }
...
class _SoilSourceOption extends StatelessWidget {
  const _SoilSourceOption({
    required this.icon,
    required this.title,
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: selected
                ? AppTokens.primary.withValues(alpha: 0.42)
        ...
        child: Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: selected
                    ...
                    : AppTokens.slate300.withValues(alpha: 0.4),
                borderRadius: BorderRadius.circular(10),
              ),
              child: Icon(icon,
                  size: 18,
                  color: selected ? AppTokens.primary : AppTokens.slate500),
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(subtitle,
                      style:
                          _bodyStyle(size: 11.5, color: AppTokens.slate500)),
                ],
              ),
            ),
            ...
            Icon(
              ...
              size: 20,
              color: selected ? AppTokens.primary : AppTokens.slate400,
            ),
          ],
        ),
      ),
...
/// Shows which recorded sample the advisory will run on, and opens the picker.
class _RecordedSampleField extends StatelessWidget {
  const _RecordedSampleField({required this.controller});

  final StationDashboardController controller;

  @override
  Widget build(BuildContext context) {
    final sample = controller.advisorySoilSample;

    if (controller.isLoadingSoilSamples) {
      return Container(
        ...
        child: Row(
          children: [
            ...
            const SizedBox(width: 12),
            Expanded(
              child: Text('Loading recorded samples…',
    ...
    final error = controller.soilSamplesError;
    if (error != null) {
      return Container(
        padding: const EdgeInsets.all(13),
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(13),
          border: Border.all(color: AppTokens.alert.withValues(alpha: 0.25)),
        ),
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Icon(Icons.error_outline_rounded,
                size: 18, color: AppTokens.alert),
            const SizedBox(width: 10),
            Expanded(
              child: Text('Could not load samples: $error',
                  style: _bodyStyle(size: 12, color: AppTokens.slate500)),
            ),
            TextButton(
              onPressed: () => controller.loadSoilSamples(),
    ...
    if (controller.soilSamples.isEmpty) {
      return Container(
        padding: const EdgeInsets.all(13),
        decoration: BoxDecoration(
        ...
        child: Row(
          ...
          children: [
            const Icon(Icons.info_outline_rounded,
                size: 16, color: AppTokens.slate500),
            const SizedBox(width: 9),
            Expanded(
              child: Text(
                'No samples recorded for this farm yet. Capture one on the '
                'Soil Sampling screen.',
                style: _bodyStyle(size: 12, color: AppTokens.slate500),
              ),
            ),
          ],
    ...
    return _SelectorButton(
      ...
      label: 'RECORDED SAMPLE',
      value: sample == null
          ? 'Choose sample'
          : '${sample.farmName.isNotEmpty ? sample.farmName : sample.cropName} · ${sample.cropName.isNotEmpty ? sample.cropName : 'Sample'} · ${_formatSampleStamp(sample.timestamp)}',
      placeholder: sample == null,
      onTap: () => _openSamplePicker(context, controller),
    );
  }
}
...
void _openSamplePicker(
  ...
  showModalBottomSheet(
    ...
    builder: (context) => _SamplePickerSheet(controller: controller),
  );
}

class _SamplePickerSheet extends StatelessWidget {
  const _SamplePickerSheet({required this.controller});

  final StationDashboardController controller;

  @override
  Widget build(BuildContext context) {
    final List<SoilSample> samples = controller.soilSamples;
    final selectedId = controller.advisorySoilSample?.id;
        ...
        return Padding(
          ...
          child: Column(
            children: [
              ...
              Row(
                children: [
                  Text('Recorded soil samples', style: _sectionTitleStyle()),
                  const Spacer(),
                  Text('${samples.length}', style: _labelStyle()),
                ],
              ),
              ...
              Expanded(
                child: ListView.separated(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    final selected = sample.id == selectedId;
                    return InkWell(
                      borderRadius: BorderRadius.circular(13),
                      onTap: () {
                        controller.setAdvisorySoilSample(sample);
                          ...
                          children: [
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        sample.farmName.isNotEmpty
                                            ? '${sample.farmName} · ${sample.cropName.isNotEmpty ? sample.cropName : 'Sample'}'
                                            : sample.label,
                                        maxLines: 1,
                                        overflow: TextOverflow.ellipsis,
                                        style: GoogleFonts.plusJakartaSans(
                                      ...
                                      Text(
                                        sample.address.isNotEmpty
                                            ? sample.address
                                            : _formatSampleStamp(sample.timestamp),
                                        maxLines: 2,
                                        overflow: TextOverflow.ellipsis,
                                        style: _bodyStyle(
                                            size: 11.5,
                            ...
                            Wrap(
                              ...
                              children: [
                                _SampleChip(
                                    label: 'N',
                                    value: _trimNum(sample.nitrogen)),
                                _SampleChip(
                                    label: 'P',
                                    value: _trimNum(sample.phosphorus)),
                                _SampleChip(
                                    label: 'K',
                                    value: _trimNum(sample.potassium)),
                                _SampleChip(
                                    label: 'pH',
                                    value: sample.ph.toStringAsFixed(1)),
                                _SampleChip(
                                    label: 'Moist',
                                    value:
                                        '${sample.moisture.toStringAsFixed(1)}%'),
                                _SampleChip(
                                    label: 'Temp',
                                    value:
                                        '${sample.temperature.toStringAsFixed(1)}°C'),
                                _SampleChip(
                                    label: 'EC', value: _trimNum(sample.ec)),
                              ],
                            ),
                          ],
                        ),
...
class _SampleChip extends StatelessWidget {
  const _SampleChip({required this.label, required this.value});
...
/// `12 Mar 2026 · 14:05`
String _formatSampleStamp(DateTime d) =>
    '${_formatDate(d)} · '
