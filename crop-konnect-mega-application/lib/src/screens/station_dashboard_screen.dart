...
import 'farm_readings_screen.dart';
import 'health_tab.dart';
import 'home_tab.dart';
import 'settings_tab.dart';
import 'soil_sampling_tab.dart';
...
class StationDashboardScreen extends StatefulWidget {
  ...
  /// The farm this dashboard details. Opened from the pre-dashboard; falls back
  /// to the primary farm when the screen is pushed directly.
  final Farm? farm;
...
class _StationDashboardScreenState extends State<StationDashboardScreen>
  ...
  /// The farm being detailed by this dashboard.
  Farm get _farm => widget.farm ?? FarmCatalog.primary;
  ...
  StationDashboardController _buildController() {
    ...
    return StationDashboardController(
      client: StationDataSourceFactory.create(
        ...
        deviceId: farm.primaryLogger?.deviceId ?? AppConfig.deviceId,
  ...
  /// Pull the farm list from Supabase, then re-resolve the controller's farm
  /// against it so no screen is left holding a stale farm object.
  ///
  /// [FarmCatalog.load] never throws and never empties the catalogue, so the
  /// worst case here is that the built-in farm stays on screen.
  Future<void> _loadFarms() async {
    await FarmCatalog.load(force: true);
    if (!mounted) return;
    _controller.syncFarms();
    setState(() {});
  }

  Future<void> _reloadFarms() => _loadFarms();

  /// Open a farm's reading dashboard as a sub-screen over the nav shell.
  void _openFarmReadings(Farm farm) {
    Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => FarmReadingsScreen(
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _QuickSettingsSheet(
        onGoToSettings: () {
          ...
          // Return to the shell first if a sub-screen is on top; a no-op when
  ...
  List<Widget> _buildTabs() {
    ...
    final tabs = <Widget>[
      HomeTab(
        onOpenFarm: _openFarmReadings,
        onReloadFarms: _reloadFarms,
      ),
      DssCenterTab(controller: _controller),
      SoilSamplingTab(controller: _controller, farm: _farm),
      ...
      builder: (context, _) {
        return Scaffold(
          ...
          body: FadeTransition(
            opacity: _entryAnim,
            child: SafeArea(
              // The shell renders immediately — Home does not depend on the
              // station, so waiting on the first reading here would hide the
              // farm list behind a spinner. Tabs that need readings show their
              // own loading state (see FarmReadingsScreen).
              child: LayoutBuilder(
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        // Tightened to keep all six destinations on one row at 360 dp.
        padding: selected
