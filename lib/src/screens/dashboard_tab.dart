...
class _StatusBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final stationName   = reading?.stationName ?? monitoring?.stationName ?? 'Field Station';
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [accentColor.withValues(alpha: 0.0), accentColor, accentColor.withValues(alpha: 0.4)],
              ),
            ),
          ),
...
class _StationNameBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      Text(
        name,
        style: _T.display(size: 26, color: _T.ink900, letterSpacing: -0.8, height: 1.1),
      ),
      ...
      children: [
        ...
        Column(
          ...
          children: [
            Text(
              statusText,
              style: _T.display(size: 16, color: accentColor, letterSpacing: -0.3),
            ),
            Text(
              statusDescription(monitoring),
              style: _T.label(size: 9.5, color: accentColor.withValues(alpha: 0.7), letterSpacing: 0.4),
            ),
          ],
        ),
      ],
  ...
  Widget build(BuildContext context) => MouseRegion(
    ...
    child: GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Icon(widget.icon, size: 17, color: _hovered ? _T.ink700 : _T.ink400),
      ),
    ),
  );
}
...
class _HeroMetricStrip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          _MapPanel(lat: lat, lng: lng, hasGps: hasGps, height: 400, reading: reading),
          Container(height: 1, color: _T.ink100),
        ],
      ),
    );
              ...
              MarkerLayer(
                markers: [
                  Marker(
                    ...
                    width: 80, height: 80,
                    child: Tooltip(
                      message: 'Moisture: ${formatMetric(widget.reading?.moist, suffix: '%')}\nTemperature: ${formatMetric(widget.reading?.temp, suffix: '°')}',
                      triggerMode: TooltipTriggerMode.tap,
                      preferBelow: false,
                      child: _RippleMarker(color: _T.red),
                    ),
                  ),
                ],
              ),
          ...
          Container(
            ...
            child: Center(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.satellite_alt_rounded, color: _T.ink400, size: 28),
                  const SizedBox(height: 8),
                  Text('Awaiting GPS…', style: _T.label(color: _T.ink400)),
                ],
              ),
            ),
          ),
        ...
        Positioned(
          ...
          child: Container(
            ...
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                const Icon(Icons.satellite_alt_rounded, color: Colors.white, size: 11),
                const SizedBox(width: 5),
                Text('LIVE HD', style: _T.label(size: 8.5, color: Colors.white, letterSpacing: 0.8)),
              ],
            ),
          ),
        ),
          ...
          children: [
            Row(
              ...
              children: [
                ...
                Text(d.label,
                    style: _T.label(
                        size: 9.5, color: d.color.withValues(alpha: 0.8),
                        letterSpacing: 0.4)),
              ],
            ),
            const SizedBox(height: 5),
            Text(d.value,
                style: _T.mono(size: 14, color: _hovered ? d.color : _T.ink900)),
          ],
        ),
      ),
    );
...
class _ResponsiveDataMatrix extends StatelessWidget {
  const _ResponsiveDataMatrix({required this.reading, required this.monitoring, this.trends});
  final SensorReading?    reading;
  final MonitoringStatus? monitoring;
            ...
            children: [
              Expanded(
                ...
                child: Column(children: [
                  _SectionLabel(title: 'LIVE SENSOR MATRIX',
              ...
              Expanded(
                flex: 44,
                child: Column(children: [
                  _SectionLabel(title: 'SOIL NUTRIENTS',
                      subtitle: 'NPK trend analysis'),
...
class _SensorDataList extends StatelessWidget {
  ...
  List<_SensorRow> _buildRows(SensorReading? r, MonitoringStatus? m) => [
    _SensorRow(icon: Icons.water_drop_rounded, label: 'Soil Moisture',
        sublabel: 'Root zone saturation',
        value: formatMetric(r?.moist, suffix: '%'),
        barValue: _norm(r?.moist, 0, 100), barColor: _T.g600,
        condition: m?.conditions['soil_moisture'],
        conditionColor: _condColor(m?.conditions['soil_moisture'])),
    _SensorRow(icon: Icons.thermostat_rounded, label: 'Soil Temperature',
        sublabel: 'Ground thermal',
        value: formatMetric(r?.temp, suffix: '°C'),
        barValue: _norm(r?.temp, 0, 60), barColor: _T.orange,
        condition: m?.conditions['temperature'],
        conditionColor: _condColor(m?.conditions['temperature'])),
    _SensorRow(icon: Icons.air_rounded, label: 'Wind Speed',
        sublabel: 'Canopy height',
        value: formatMetric(r?.ws, suffix: ' m/s'),
        barValue: _norm(r?.ws, 0, 30), barColor: _T.sky,
        condition: m?.conditions['wind'],
        conditionColor: _condColor(m?.conditions['wind'])),
    _SensorRow(icon: Icons.explore_rounded, label: 'Wind Direction',
        sublabel: 'Compass heading',
        value: r == null ? '--' : '${formatNumber(r.wdDeg)}° ${r.wdDir ?? ''}'.trim(),
        barValue: _norm(r?.wdDeg?.toDouble(), 0, 360), barColor: _T.ink400,
        condition: null, conditionColor: _T.ink400),
    _SensorRow(icon: Icons.wb_sunny_rounded, label: 'UV / Solar',
        sublabel: 'Photosynthetic radiation',
        value: formatMetric(r?.solar),
        barValue: _norm(r?.solar, 0, 1200), barColor: _T.amber,
        condition: m?.conditions['uv'],
        conditionColor: _condColor(m?.conditions['uv'])),
    _SensorRow(icon: Icons.grain_rounded, label: 'Rainfall',
        sublabel: 'Accumulated precipitation',
        value: formatMetric(r?.rain, suffix: ' mm'),
        barValue: _norm(r?.rain, 0, 50), barColor: _T.violet,
        condition: null, conditionColor: _T.ink400),
    _SensorRow(icon: Icons.bolt_rounded, label: 'Electrical Conductivity',
        sublabel: 'Soil salinity index',
        value: formatMetric(r?.ec, suffix: ' µS/cm'),
        barValue: _norm(r?.ec, 0, 2000), barColor: _T.ink700,
        condition: null, conditionColor: _T.ink400),
    _SensorRow(icon: Icons.science_rounded, label: 'Soil pH',
        sublabel: 'Acid–alkaline balance',
        value: formatNumber(r?.ph, digits: 2),
        barValue: _norm(r?.ph, 0, 14), barColor: _T.teal,
        condition: null, conditionColor: _T.ink400),
  ];
...
class _SensorRow extends StatefulWidget {
  const _SensorRow({
    ...
    required this.barValue,      required this.barColor,
    required this.condition,     required this.conditionColor,
  });
  final IconData icon;
...
class _SensorRowState extends State<_SensorRow> {
  ...
  Widget build(BuildContext context) => MouseRegion(
    ...
    child: AnimatedContainer(
      ...
      color: _hovered ? widget.barColor.withValues(alpha: 0.03) : Colors.transparent,
          ...
          children: [
            ...
            Expanded(
              child: Padding(
                ...
                child: _AnimatedBar(value: widget.barValue, color: widget.barColor),
              ),
            ),
...
class _NutrientTrendGraph extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasData = nSeries.isNotEmpty || pSeries.isNotEmpty || kSeries.isNotEmpty;
        ...
        children: [
          ...
          Row(children: [
            Expanded(child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('NPK Trends', style: _T.display(size: 22, letterSpacing: -0.6)),
            ...
            if (total > 0)
              Container(
                ...
                decoration: BoxDecoration(
                  color: _T.ink50, borderRadius: BorderRadius.circular(10),
          ...
          // ── Legend row ──
          Row(children: [
            _NPKLegendChip(symbol: 'N', label: 'Nitrogen', value: n, color: _nColor),
            const SizedBox(width: 8),
            _NPKLegendChip(symbol: 'P', label: 'Phosphorus', value: p, color: _pColor),
            const SizedBox(width: 8),
            _NPKLegendChip(symbol: 'K', label: 'Potassium', value: k, color: _kColor),
          ]),
          const SizedBox(height: 20),
          ...
          SizedBox(
            ...
            child: hasData
                ...
                : Center(child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Icon(Icons.show_chart_rounded, color: _T.ink200, size: 40),
                      const SizedBox(height: 8),
                      Text('Awaiting trend data…', style: _T.label(color: _T.ink400)),
                    ],
                  )),
          ),
          ...
          if (alerts.isNotEmpty) ...[
            ...
            Text('SOIL ADVISORY', style: _T.label(size: 9.5, color: _T.ink400, letterSpacing: 1.2)),
            const SizedBox(height: 10),
            ...alerts.map((a) => Padding(
              padding: const EdgeInsets.only(bottom: 8),
              child: _AdvisoryAlert(
                icon: a.icon, title: a.title, message: a.message,
  ...
  Widget _buildChart(List<TrendPoint> nS, List<TrendPoint> pS, List<TrendPoint> kS) {
    ...
    return LineChart(LineChartData(
      ...
      gridData: FlGridData(
        ...
        getDrawingHorizontalLine: (v) => FlLine(color: _T.ink100, strokeWidth: 0.8),
      ),
      titlesData: FlTitlesData(
        ...
        bottomTitles: AxisTitles(sideTitles: SideTitles(
          showTitles: true, reservedSize: 22, interval: _xInterval(nS.length),
          getTitlesWidget: (v, _) {
            final i = v.toInt();
            if (i < 0 || i >= nS.length) return const SizedBox.shrink();
            final h = nS[i].timestamp.hour;
            return Text('${h}h', style: _T.label(size: 8, color: _T.ink400, letterSpacing: 0));
          },
        )),
      ),
        ...
        touchTooltipData: LineTouchTooltipData(
          ...
          getTooltipItems: (spots) => spots.map((s) {
            ...
            return LineTooltipItem(
              ...
              GoogleFonts.plusJakartaSans(
                color: colors[s.barIndex], fontSize: 11, fontWeight: FontWeight.w700),
            );
          }).toList(),
        ),
      ),
  ...
  LineChartBarData _line(List<TrendPoint> pts, Color color) {
    return LineChartBarData(
      spots: pts.asMap().entries.map((e) =>
          FlSpot(e.key.toDouble(), e.value.value ?? 0)).toList(),
  ...
  List<_Advisory> _buildAdvisories(double n, double p, double k) {
    ...
    if (n == 0 && p == 0 && k == 0) {
      list.add(_Advisory(Icons.info_outline_rounded, 'No Data',
          'Nutrient sensor readings are unavailable. Check probe connection.',
          _T.ink500, _T.ink50));
      return list;
    }
    if (n < 10) {
      list.add(_Advisory(Icons.warning_amber_rounded, 'Low Nitrogen',
          'N is below 10 mg/kg — consider urea or ammonium-based fertilizer.',
          const Color(0xFFEF4444), const Color(0xFFFEF2F2)));
    }
    if (n > 40) {
      list.add(_Advisory(Icons.eco_rounded, 'High Nitrogen',
          'N exceeds 40 mg/kg — reduce nitrogen inputs to prevent leaching.',
          _T.amber, const Color(0xFFFFFBEB)));
    }
    if (p < 0.5) {
      list.add(_Advisory(Icons.warning_amber_rounded, 'Low Phosphorus',
          'P is below 0.5 mg/kg — apply DAP or SSP fertilizer.',
          const Color(0xFFEF4444), const Color(0xFFFEF2F2)));
    }
    if (k < 0.8) {
      list.add(_Advisory(Icons.warning_amber_rounded, 'Low Potassium',
          'K is below 0.8 mg/kg — apply potash (MOP/SOP) to improve crop quality.',
          const Color(0xFFEF4444), const Color(0xFFFEF2F2)));
    }
    if (list.isEmpty) {
      list.add(_Advisory(Icons.check_circle_outline_rounded,
          'Balanced Nutrients',
...
class _Advisory {
  const _Advisory(this.icon, this.title, this.message, this.color, this.bgColor);
  final IconData icon;
...
class _NPKLegendChip extends StatelessWidget {
  const _NPKLegendChip({
    required this.symbol, required this.label,
    ...
    child: Container(
      ...
      child: Column(children: [
        Container(
          ...
          decoration: BoxDecoration(
            color: color.withValues(alpha: 0.14), borderRadius: BorderRadius.circular(6)),
          child: Text(symbol, style: _T.body(size: 10, color: color, weight: FontWeight.w900)),
        ),
        const SizedBox(height: 5),
        Text(value > 0 ? value.toStringAsFixed(1) : '—',
            style: _T.mono(size: 15, color: _T.ink900)),
        const SizedBox(height: 1),
        Text(label, style: _T.label(size: 7.5, color: _T.ink400, letterSpacing: 0.3)),
      ]),
    ),
  );
}
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: _T.body(size: 12, color: color, weight: FontWeight.w800)),
          const SizedBox(height: 2),
          Text(message, style: _T.body(size: 11, color: _T.ink500, weight: FontWeight.w600)),
        ],
      )),
    ]),
  );
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
    ...
    builder: (_, __) => Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: widget.color.withValues(alpha: 0.25 + _anim.value * 0.12)),
      ),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
...
class _RippleMarker extends StatefulWidget {
  const _RippleMarker({required this.color});
  final Color color;
...
Color _statusAccent(String status) {
  switch (status.toLowerCase()) {
    case 'optimal': case 'good':    return _T.g500;
    case 'warning': case 'caution': return _T.amber;
    case 'critical': case 'alert':  return _T.red;
    default:                        return _T.ink400;
  }
}
...
Color _condColor(String? condition) {
  ...
  switch (condition.toLowerCase()) {
    case 'optimal': case 'good':    return _T.g600;
    case 'warning': case 'caution': return _T.amber;
    case 'critical': case 'alert':  return _T.red;
    default:                        return _T.ink500;
  }
}
